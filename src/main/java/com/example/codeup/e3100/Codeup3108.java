package com.example.codeup.e3100;
...
class Student {
    ...
    public Student(String code, int testNum, String name) {
        this.code = code;
...
public class Codeup3108 {
    ...
    private boolean isExisted(Student pStudent) {
        ...
        for (Student student : students) {
            if (pStudent.getTestNum() == student.getTestNum()) return true;
        }
        return false;
    }
    ...
    private void printStudents() {
        for (Student student : students) {
            System.out.printf("%s %s %s\n", student.getCode(), student.getTestNum(), student.getName());
        }
    }
    ...
    private void printSpecificStudents(int[] arr) {
        ...
        Collections.sort(students, new Comparator<Student>() {
            ...
            public int compare(Student o1, Student o2) {
                return o1.getTestNum() - o2.getTestNum();
            }
        } );
    ...
    private void deleteStudent(Student pStudent) {
        for (int i = 0; i < students.size(); i++) {
            if(students.get(i).getTestNum() == pStudent.getTestNum()) students.remove(i);
        }
    }
